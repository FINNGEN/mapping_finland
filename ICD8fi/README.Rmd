...
icd8fi_source_vocabulary <- read_csv("./1_source_vocabulary/icd8fi_for_source_vocabulary.csv", col_types = col_types)

col_types = cols( code = col_character(),
                  .default = col_number()
                  )
icd8fi_freqs <- read_csv("./3_freq_of_source_codes/freq_total.csv", col_types = col_types)
...
#                   )
# 
# icd8fi_after_usagi <- read_csv("./2_mapping_to_standard/USAGI/icd8fi_after_usagi.csv", col_types = col_types) %>% 
#   mutate(name_en_icd9 = `ADD_INFO:possible_english_name`)# cant get the encoding rigth
icd8fi_after_usagi <- icd8fi_source_vocabulary %>% 
...
```{r}
icd8fi_source_vocabulary %>% count(name_en_source, sort = T)
```

Source table with added translation has been formatted to be similar to OMOP in  [1_source_vocabulary/icd8fi_for_source_vocabulary.csv](1_source_vocabulary/icd8fi_for_source_vocabulary.csv). 
...
```{r}
n_codes <- icd8fi_after_usagi %>% distinct(code) %>% nrow()
n_codes_accepted <- icd8fi_after_usagi %>% filter(mappingStatus=="APPROVED") %>% distinct(code) %>% nrow()
```
...
```{r}
a <- icd8fi_after_usagi %>% distinct(code, mappingStatus, sourceFrequency)
...
# 
# 
# icd8fi_after_usagi %>% filter(mappingStatus=="UNCHECKED") %>% arrange(-sourceFrequency) %>% select(code,  sourceFrequency, matchScore, sourceName, name_en_icd9) %>%  head(N_TOPS)
```
...
db_names <- icd8fi_freqs %>% names %>% setdiff(c("code", "freq_total")) %>% str_replace("freq_","")
...
#assess_db <- function(i){
 # db_name <- db_names[i]
  ...
  save_db_missing_path <- str_c("./3_freq_of_source_codes/",db_name,"_not_in_icd8fi.csv")
  ...
  my_print_line("**How many codes labeled as icd8fi in ", db_name," are not in the icd8fi standard?**")
  ...
  my_print_line("**Status of the icd8fi codes in", db_name, "**")
