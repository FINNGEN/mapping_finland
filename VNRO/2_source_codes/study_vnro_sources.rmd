...
freq_vnro_finngen <- read_csv("../1_source_codes_freq/freq_purch_finngen.csv") %>% select(-X1) 
...
ari <- read_excel("./vnr_vahvuus_pkoko_Arille_210220-1.xlsx")
```
...
ari <-  ari %>% mutate(vnr = as.integer(vnr))%>% 
  rename(atc = ATC, vnr = vnr)
...
joined <- bind_rows(
  ...
  ari %>% select(atc,vnr) %>% mutate(ss="ari")
) %>% 
