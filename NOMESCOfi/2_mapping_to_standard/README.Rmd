...
nomescofi_mapping <- nomescofi_mapping %>% 
...
write_csv(nomescofi_mapsto, "CONCEPT_RELATIONSHIP_NOMESCOfi.csv")
...
## Recalculate domain 
```{r}
nomescofi_domain <- nomescofi_mapsto %>% 
  ...
  mutate(domain = str_replace(domain, "Observation Procedure", "Obs/Procedure" ))
...
nomescofi_concept_new <- left_join(
  nomescofi_concept, 
  nomescofi_domain, 
  by="concept_id"
) %>% 
...
#overwrite
write_csv(nomescofi_concept_new, "../1_source_vocabulary/CONCEPT_NOMESCOfi.csv")

```

```{r}
nomescofi_concept_new %>% count(domain_id, sort=T)
```
