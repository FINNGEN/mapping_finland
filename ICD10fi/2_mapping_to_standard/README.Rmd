...
ICD10fi_2code_1code_name_match <- 
  inner_join(
    ICD10fi_2code_1code_not_matched %>% select(-concept_id_who, -concept_code_who),
    ICD10fi_1code_full_match  %>% select(concept_id, tmp_name_fi, concept_code) %>% 
      rename(concept_id_who = concept_id, concept_code_who = concept_code), 
    by = c("tmp_name_fi")
  ) %>% 
...
ICD10fi_2code_1code_not_matched <- ICD10fi_2code_1code_not_matched %>% filter(!(concept_id %in% ICD10fi_2code_1code_name_match$concept_id))
...
icd10fi_mapsto_icd10who <- left_join(
  ICD10fi_mapped_icd10who %>% select(concept_id, concept_id_who, tmp_mapping_type, tmp_code_class) %>% rename(concept_id_1 = concept_id), 
...
) %>% select(1, 4:8, tmp_mapping_type, tmp_code_class)
