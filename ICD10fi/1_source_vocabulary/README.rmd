...
icd10fi_source <- left_join(
  icd10fi_source %>%  select(-name_en), 
  icd10fi_source_google_tralations, 
  by="code"
)

#tmp fix add name_fi
write_csv(
  icd10fi_source %>% select(code, name_en_source, name_fi, name_en) %>% mutate(better_name_en=""), 
...
icd10fi_concept_class <-
  bind_rows(
    ...
    tibble(
      concept_class_id = "ICD10fi code", 
      concept_class_name = "ICD10fi code",
    ...
    tibble(
      concept_class_id = "ICD10fi 2 code [autogen]", 
...
icd10fi_concept <- icd10fi_source_extended %>% 
  transmute(
    ...
    concept_class_id = case_when(
      code_class=="2 code generated" ~ "ICD10fi 2 code [autogen]", 
