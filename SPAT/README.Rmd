...
spat_source_vocabulary <- read_csv("./1_source_vocabulary/spat_for_source_vocabulary.csv", col_types = col_types)

col_types = cols( code = col_character(),
                  .default = col_number()
                  )
spat_freqs <- read_csv("./3_freq_of_source_codes/freq_total.csv", col_types = col_types)
...
#                   )
# 
# spat_after_usagi <- read_csv("./2_mapping_to_standard/USAGI/spat_after_usagi.csv", col_types = col_types) %>% 
#   mutate(name_en_icd9 = `ADD_INFO:possible_english_name`)# cant get the encoding rigth
spat_after_usagi <- spat_source_vocabulary %>% 
...
```{r}
spat_source_vocabulary %>% count(name_en_source, sort = T)
```
...
```{r}
n_codes <- spat_after_usagi %>% distinct(code) %>% nrow()
n_codes_accepted <- spat_after_usagi %>% filter(mappingStatus=="APPROVED") %>% distinct(code) %>% nrow()
```
...
```{r}
a <- spat_after_usagi %>% distinct(code, mappingStatus, sourceFrequency)
...
# 
# 
# spat_after_usagi %>% filter(mappingStatus=="UNCHECKED") %>% arrange(-sourceFrequency) %>% select(code,  sourceFrequency, matchScore, sourceName, name_en_icd9) %>%  head(N_TOPS)
```
...
db_names <- spat_freqs %>% names %>% setdiff(c("code", "freq_total")) %>% str_replace("freq_","")
...
#assess_db <- function(i){
 # db_name <- db_names[i]
  ...
  save_db_missing_path <- str_c("./3_freq_of_source_codes/",db_name,"_not_in_spat.csv")
  ...
  my_print_line("**How many codes labeled as spat in ", db_name," are not in the spat standard?**")
  ...
  my_print_line("**Status of the spat codes in", db_name, "**")
