...
hpn_concept <- read_csv("./1_source_vocabulary/CONCEPT_HPN.csv") %>% 
  filter(domain_id!="Metadata")

hpn_mapsto <- read_csv("../ATC/2_mapping_to_standard/CONCEPT_RELATIONSHIP_ATC.csv") %>% 
  filter(FALSE) ## read an other table to keep same colums

hpn_freqs <- read_csv("./3_freq_of_source_codes/freq_total.csv",
  col_types = cols(
    code = col_character(),
...
```{r}
hpn_concept %>% count(tmp_name_en_source, sort = T)
```

Source table with added translation has been formatted to be similar to OMOP in  [1_source_vocabulary/hpn_for_source_vocabulary.csv](1_source_vocabulary/hpn_for_source_vocabulary.csv). 



## Progess in number of codes
```{r}
n_codes <- hpn_concept %>% nrow()
n_codes_accepted <- hpn_mapsto %>% distinct(concept_id_1) %>% nrow()
```
...
#write_csv(db_statuses, "status_table.csv" )
 database_coverage_md("HPN", hpn_concept, hpn_mapsto, hpn_freqs) 
